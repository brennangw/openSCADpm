...
function uninstall {
#only do one uninstall


	if [ ! -z $libLoc ] && [ "$libLoc" != "/" ] && [ ! -z $1 ] && [ ! -z $2 ]; then
		libLoc=$(cat /usr/local/lib/ospmLibSettings)
		if [ "$1" == "rec" ]; then
			#check if target is required by another package
				#if it is stop
				#else if it is not required by another package
					#build map
						#for each p of dir
							#if not target
								#add to map
			...
			#check to see if other packages require this one.
			for d in $( ls -d $libLoc$slash*/ ) ; do #this wont work :(
				# if [[ -L "${f%/}" ]]; then
				echo "$d"
				# fi
				if grep -Fxq "$2 $3 $4" $d$slash$deps && "$d" != "$5-$6-$7"
				then
					echo "$2-$3-$4 is being used in $d"
					beingUsed=true
				fi
			done
		#if other packages do require this one then DONE.
		if $beingUsed ; then
			echo "$2-$3-$4 not uninstalled"
			return
			#if they do not then run full delete for each pacakge.
		else

			while read -r dep; do
				source opsm.sh uninstall full $dep $1 $2 $3
			done < $libLoc$slash$1-$2-$3$slash$deps

			rm -rf $libLoc$slash$1-$2-$3
		fi

	elif [ "$1" == "safe" ]; then
		beingUsed=false
		for d $( ls -d $libLoc$slash*/ ) ; do #this wont work :(
		# if [[ -L "${f%/}" ]]; then
		echo "$d"
		# fi
		if grep -Fxq "$2 $3 $4" $d$slash$deps
		then
			echo "$2-$3-$4 is being used in $d"
			beingUsed=true
		fi
